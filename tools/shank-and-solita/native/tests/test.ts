...
describe("Car Rental Service", () => {
	...
	const program = loadKeypairFromFile(
		"./program/target/deploy/car_rental_service-keypair.json",
	);
	...
	async function createCar(car: AddCarArgs): Promise<PublicKey> {
		...
		const carData = await Car.fromAccountAddress(
			connection,
			carAccountPublicKey,
		);
	...
	async function printRentalDetails(
		rentalPublicKey: PublicKey,
		carPublicKey: PublicKey,
	) {
		const rentalData = await RentalOrder.fromAccountAddress(
			connection,
			rentalPublicKey,
		);
