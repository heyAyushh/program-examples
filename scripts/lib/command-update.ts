...
export function commandUpdate(path = ".", packageNames: string[] = []) {
	...
	let total = 0;
	for (const pkgName of pkgNames.filter(
		(pkgName) => packageNames.length === 0 || packageNames.includes(pkgName),
	)) {
		...
		for (const file of files) {
			const [changed, content] = changePackageVersion(
				file,
				pkgName,
				`^${npmVersion}`,
			);
		...
		console.log(
			p.green(` -> Updated ${count} files with ${pkgName} ${npmVersion}`),
		);
	}
